...
DateA = np.array(datafr.columns[5:]) #Date in format: no.of days since 1-January-0000
Pr_all = get_pr_data(datafr) #sorted by date i.e 1st row is data of all stns for 1st date.
Stn_coords = combine_coords(Long,Lat) #Coordinates of rainfall stn
...
#df_stations = pd.json_normalize(requests.get("https://api.data.gov.sg/v1/environment/rainfall").json()["metadata"]["stations"])
...
# Re-make array to [id,Date,Rainfall,Stn_name]
dates = DateA
dx=pd.DataFrame(datafr) #convert csv to dataframe
df = dx.melt(id_vars=["id","Station",], var_name="Date", value_name="Rainfall (mm)") #to restructure the dataframe
df["Rainfall (mm)"] = pd.to_numeric(df["Rainfall (mm)"], errors="coerce") #Handling NaNs

# number of meters
STEP = 400 #grid resolution size in nxn m
...
gdf_rainfall = gpd.GeoDataFrame(
    df_stations.merge(df, on="id").assign(geometry=lambda d: gpd.points_from_xy(d["location.longitude"], d["location.latitude"]))
    .drop(columns=["location.latitude", "location.longitude"]),crs=gdf_sg.crs,)

# Thiessen polygon-weather station to nearest grid
gdf_grid_rainfall = gpd.sjoin_nearest(gdf_grid, gdf_rainfall).drop(columns=["Description", "index_right"])
print(gdf_grid_rainfall.head())
