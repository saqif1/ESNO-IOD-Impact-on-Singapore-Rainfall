...
df_year['ENSO'] = np.where(df_year['NINO 3.4 SST Anomalies'] > nino_sd, 'EN',
                            np.where(df_year['NINO 3.4 SST Anomalies'] < -nino_sd, 'LN', 'Neutral'))

# Create a new column for IOD classifications based on +1 standard deviation of DMI
df_year['IOD'] = np.where(df_year['DMI'] > dmi_sd, '+IOD',
                           np.where(df_year['DMI'] < -dmi_sd, '-IOD', 'Neutral'))

# Calculate the long-term average precipitation
long_term_avg_precip = df['Daily Average Precipitation'].mean()

# Calculate the Rainfall Anomaly as a percentage of departure from the long-term average
df_year['Rainfall Anomaly'] = ((df_year['Daily Average Precipitation'] - long_term_avg_precip) / long_term_avg_precip) * 100
...
#pivot_table.to_csv('focused_pivot_table_enso_iod.csv')
print(pivot_table)
print("The +1SD value for NINO 3.4 SST Anomalies is:", nino_sd)
print("The +1SD value for DMI is:", dmi_sd)

#print(df_year.columns)
#df_year.to_csv('focused_years_classified.csv')
