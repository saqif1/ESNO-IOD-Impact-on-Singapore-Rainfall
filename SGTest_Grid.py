...
df = dx.melt(id_vars=["id","Station","Longitude","Latitude","Elevation"], var_name="Date", value_name="Rainfall (mm)") #to restructure the dataframe cannot leave any header blank categorise them correctly
...
gdf_rainfall = gpd.GeoDataFrame(
    ...
    crs=gdf_sg.crs,
) #Add lat and lon columns into 1 coordinate. Then, remove the .drop individual columns

# nearest rainfall station to grid mapping
gdf_grid_rainfall = gpd.sjoin_nearest(gdf_grid, gdf_rainfall).drop(columns=["Description", "index_right"])### NaN=0???
